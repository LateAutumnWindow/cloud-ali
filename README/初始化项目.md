...
1. 安装 mysql
  1.1 创建一个 nacos 数据库
  1.2 执行 nacos-mysql.sql 文件创建 nacos 所需的表
...
1. Docker 安装：
	...
	1.4 启动命令：
		查看最新启动Docker命令参数： https://hub.docker.com/r/nacos/nacos-server
...
```txt
// 以Order服务为例
// ********* 如果是用 yaml 配置 Data ID 一定要以 .yaml 结尾不然取不到配置信息 *********
...
项目中的 seata.conf
client {
    application.id = raw-order									-- A1
    transaction.service.group = my_order_test_tx_group			-- B1
}
...
seata:
  ...
  config:
    ...
    nacos:
      ...
      username: "nacos"
      password: "nacos"
  enable-auto-data-source-proxy: true							-- B1
