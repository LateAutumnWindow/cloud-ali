...
import com.yan.cloud.pojo.LoginRequest;
import com.yan.cloud.pojo.LoginResponse;
import com.yan.cloud.pojo.RefreshRequest;
...
public class LoginController {
  ...
  public CommonResult login(@RequestBody @Validated LoginRequest request, BindingResult bindingResult) {
      if (bindingResult.hasErrors()) {
          return CommonResult.error(ResponseCodeEnum.PARAMETER_ILLEGAL.getCode(), ResponseCodeEnum.PARAMETER_ILLEGAL.getMessage());
      }

      String username = request.getUsername();
      String password = request.getPassword();
      //  假设查询到用户ID是1001
      String userId = "1001000000000000";
...
//        String key = userId;
      ...
      return CommonResult.error(ResponseCodeEnum.LOGIN_ERROR.getCode(), ResponseCodeEnum.LOGIN_ERROR.getMessage());
  }
   ...
   */
  @GetMapping("/logout")
  public CommonResult logout(@RequestParam("userId") String userId) {
      HashOperations<String, String, String> hashOperations = stringRedisTemplate.opsForHash();
//      String key = userId;
  ...
  /**
   ...
   public CommonResult refreshToken(@RequestBody @Validated RefreshRequest request, BindingResult bindingResult) {
...
//       String originalRefreshToken = hashOperations.get(key, "refreshToken");
       ...
       if (StringUtils.isBlank(originalRefreshToken) || !originalRefreshToken.equals(refreshToken)) {
           return CommonResult.error(ResponseCodeEnum.REFRESH_TOKEN_INVALID.getCode(), ResponseCodeEnum.REFRESH_TOKEN_INVALID.getMessage());
       }
